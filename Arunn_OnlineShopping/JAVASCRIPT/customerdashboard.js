(function() {
    ...
    $(document).ready(function() {
        ...
        $.ajax({
            url: 'https://firestore.googleapis.com/v1/projects/e-commerce-adbd7/databases/(default)/documents/Inventory',
            type: 'GET',
            success: function(response) {
...
// Confirm payment using credit
const confirmation = confirm(`Confirm payment using credit? Total Price: $${totalPrice.toFixed(2)}`);
if (confirmation) {
                ...
                inventoryItems.forEach(inventoryItem => {
                    ...
                    if (cartProduct) {
                        ...
                        if (newQuantity >= 0) {
                            ...
                            $.ajax({
                                url: `https://firestore.googleapis.com/v1/projects/e-commerce-adbd7/databases/(default)/documents/Inventory/${inventoryId}`,
                                type: 'PATCH',
                                contentType: 'application/json',
                                data: JSON.stringify({
    ...
    function createOrder() {
        ...
        if (customerId) {
            ...
            .then(categoryNames => {
                ...
                $.ajax({
                    url: 'https://firestore.googleapis.com/v1/projects/e-commerce-adbd7/databases/(default)/documents/Purchase',
                    type: 'POST',
                    contentType: 'application/json',
...
function showOrderHistory() {
    const orderHistoryContainer = $('#orderHistoryContainer');
    if (orderHistoryContainer.is(':visible')) {
        // If the order history container is visible, hide it
        orderHistoryContainer.hide();
    } else {
        // If the order history container is hidden, show it
        // Retrieve customer ID from local storage
        const customerId = localStorage.getItem('uid');
        if (customerId) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    const customerOrders = orders.filter(order => order.fields['Customer ID'].stringValue === customerId);
                    if (customerOrders.length > 0) {
                        // Display order details
                        const orderDetails = customerOrders.map(order => {
                            // Logic to format order details
                            // Assuming order details are in HTML format
                            return `<p>${order.fields['I Name'].stringValue}: $${order.fields['Price'].integerValue}</p>`;
                        }).join('');
                        // Display order details in the orderHistoryContainer
                        orderHistoryContainer.html(orderDetails);
                    } else {
                        // No orders found message
                        orderHistoryContainer.html('<p>No orders found for this customer.</p>');
                    }
                    // Show the order history container after updating its content
                    orderHistoryContainer.show();
                },
                error: function(error) {
                    console.error('Error fetching order history:', error);
                    // Display error message if fetching order history fails
                    orderHistoryContainer.html('<p>Error fetching order history. Please try again later.</p>');
                    // Show the order history container with the error message
                    orderHistoryContainer.show();
                }
            });
        } else {
